...
class RVOMiacIncoming(gym.Env):
    ...
    def _get_obs(self):
        """Gets the observation for agent 0 with padding in neighbor data."""
        ...
        agent_neighbors = self.sim.get_neighbors_data(0)  # Assuming it returns a list of floats
        ...
        if len(agent_neighbors) < expected_neighbors_data_length:
            ...
            # In case there are more data (may not be necessary), truncate
    ...
    def step(self, action):
        """Takes a step in the simulation with the action as deviation."""
...
if __name__ == "__main__":
    env = RVOMiacIncoming('./simulator/worlds/miac/incoming/incoming_level_0.yaml', render_mode='rgb')
