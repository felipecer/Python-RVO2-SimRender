...
def train():
    vec_env = make_vec_env(RVOMiacIncoming, n_envs=64, env_kwargs={
    ...
    model = PPO("MlpPolicy", vec_env,  n_steps=1024, verbose=1, device='cpu',
                tensorboard_log="./tests/incoming/logs/level_0/")
    model.learn(total_timesteps=1000000, progress_bar=True)
    model.save("./tests/incoming/logs/saves/ppo_rvo_miac_a_lite_test1")
    print("Entrenamiento terminado")
    del model


def test():
    vec_env = make_vec_env(RVOMiacIncoming, n_envs=1, env_kwargs={
